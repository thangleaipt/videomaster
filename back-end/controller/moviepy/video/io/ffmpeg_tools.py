...
def ffmpeg_extract_subclip(filename, t1, t2, targetname=None):
    ...
    cmd = [get_setting("FFMPEG_BINARY"),"-y",
           "-ss", "%0.2f"%t1,
           "-i", filename,  # Thay đổi từ "video.mp4" thành filename để sử dụng tệp nguồn được chuyển đầu vào
           "-t", "%0.2f"%(t2-t1),
           "-an",  # Thêm tùy chọn để loại bỏ âm thanh ("-an" nghĩa là no audio)
           "-vcodec", "copy", targetname]
    print(cmd)
    subprocess_call(cmd)
