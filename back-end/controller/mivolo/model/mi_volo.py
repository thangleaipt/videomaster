...
from controller.mivolo.model.create_timm_model import create_model
from controller.mivolo.structures import PersonAndFaceCrops, PersonAndFaceResult
...
class MiVOLO:
    ...
    def predict_face(self, face_image):
        output = self.inference(face_image)
        return output
    ...
    def prepare_crops(self, image: np.ndarray, detected_bboxes: PersonAndFaceResult):
        ...
        faces_input = prepare_classification_images(faces_crops, self.input_size, self.data_config["mean"], self.data_config["std"], device=self.device)
    ...
    def prepare_crops_insightface(self, image: np.ndarray, faces_crops: np.ndarray):
